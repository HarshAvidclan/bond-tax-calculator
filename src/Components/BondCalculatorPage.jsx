...
export default function BondCalculatorPage() {
  ...
  const taxAmount = React.useMemo(
    () => Math.round((totalInterest * tax) / 100),
    [totalInterest, tax]
  );
  const netMaturity = React.useMemo(
    () => maturity - taxAmount,
    [maturity, taxAmount]
  );
  const netGain = React.useMemo(
    () => netMaturity - principal,
    [netMaturity, principal]
  );
  const netPercent = React.useMemo(
    () => (netGain / principal) * 100,
    [netGain, principal]
  );
  const annualized = React.useMemo(
    () => (Math.pow(netMaturity / principal, 1 / years) - 1) * 100,
    [netMaturity, principal, years]
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-12 font-sans">
      <Helmet>
        ...
        <meta
          name="description"
          content="Calculate net maturity and after-tax returns for cumulative corporate bonds. Interactive sliders for principal, rate, tenure and tax. SEO friendly single-page app."
        />
        <meta
          name="keywords"
          content="bond calculator, after tax bond return, LTCG calculator, corporate bonds, maturity calculator, bond tax"
        />
      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          <h1 className="text-3xl md:text-4xl font-bold text-slate-800">
            Bond Net Return Calculator
          </h1>
          <p className="mt-2 text-slate-600">
            Enter your investment details and see gross &amp; net maturity, tax,
            percentage increase and annualized return.
          </p>
        </motion.header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <motion.div
            ...
            className="col-span-1 bg-white p-6 rounded-2xl shadow-md"
          >
            ...
            <p className="text-xs text-slate-500 mt-4">
              Assumes annual compounding and tax applied only on interest (gain)
              at maturity.
            </p>
          </motion.div>
          ...
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="col-span-1 md:col-span-2 bg-white p-6 rounded-2xl shadow-md"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-lg border">
                ...
                <div className="text-sm text-slate-600 mt-1">
                  (After deducting tax on interest)
                </div>
              </div>
            ...
            <div className="mt-6">
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-600">
                ...
                <li>
                  <strong>Gross Interest:</strong> {fmt(totalInterest)}
                </li>
                <li>
                  <strong>Tax ({tax}%):</strong> {fmt(taxAmount)}
                </li>
                <li>
                  <strong>Net maturity:</strong> {fmt(netMaturity)}
                </li>
              </ul>
            </div>
            ...
            <div className="mt-6 flex gap-3">
              ...
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  const csv = `Principal,Rate,Years,Tax,GrossMaturity,NetMaturity\n${principal},${rate},${years},${tax},${maturity},${netMaturity}`;
                  const blob = new Blob([csv], { type: "text/csv" });
                  const url = URL.createObjectURL(blob);
                  const a = document.createElement("a");
                  a.href = url;
                  a.download = "bond-calculation.csv";
                  a.click();
        ...
        <footer className="mt-10 text-sm text-slate-500">
          <p>
            Note: This calculator assumes annual compounding and that tax is
            applied at maturity only on interest. It is a guide — consult a tax
            advisor for exact ITR treatment and add cess where applicable.
          </p>
        </footer>
      </div>
    </div>
